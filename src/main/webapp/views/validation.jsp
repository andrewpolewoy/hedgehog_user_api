...
<div class="block">
    <c:choose>
        ...
        <c:when test="${requestScope.userMessage}">
            <p style="color:black;">${requestScope.message}</p>
        </c:when>

        <c:otherwise>
            <h1 class="h3 mb-3 fw-normal">Проверьте пользователя</h1>
        </c:otherwise>
    </c:choose>
    <form action="${pageContext.request.contextPath}/validation" method="post">
        ...
        <button class="w-100 btn btn-sm btn-outline-dark" type="submit">Валидация</button>
